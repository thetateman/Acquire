...
const gameMessages = {
    registerGameMessageHandlers: function(games, io, sock, userStatuses, usersInLobby, verbose){
        ...
        */
        ...
        //sock.on('historicalGameAction', this.historicalGameActionHandler(games, sock, verbose)) //moved to client
        /*
    ...
    gameHistoryRequestHandler: (games, io, sock) => async (gameID) => {
        if(!(/^[\x21-\x3B\x61-\x7A]{1,99}$/.test(gameID))){
            return false;
        }
        let matchingGame = await GameModel.findById(gameID); 
        if(!matchingGame){
            return false;
        }
        sock.request.session.lastKnownLocation = `game${gameID}`;
    ...
    gameActionHandler: (games, io, sock, verbose) => ({game_id, updateType, updateData}) => {
        ...
        if(!(game_id && updateType && updateData)){
            return false;
        }
    ...
    // --------------Historical game functions (moved to client)--------------------
