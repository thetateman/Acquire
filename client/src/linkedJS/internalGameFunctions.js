(()=>{
    window.internalGameFunctions = {
        ...
        genTileBank: function(){
            ...
            tileBank = window.sharedGameFunctions.shuffleArray(tileBank);
            return tileBank;
        },
        ...
        tilePlacer: function(board, chains, share_prices, active_merger, x, y, game, gameStarted=true){
            ...
            let connectingChains = window.sharedGameFunctions.getNeighbors(board, x, y); // List of unique chains adjacent to new tile
            ...
            if(connectingChains.length > 0){
                if(connectingChains.every((element) => element === 's')){
                    ...
                    if(connectingTrueChains.length === 1){
                        ...
                        let {largestChains, remainingChain, elimChains, playersDisposing} = window.sharedGameFunctions.getMergerInfo(game, connectingTrueChains);
        ...
        endTurn: function(game){
            if(game.state.no_playable_tile_turns === 0){ // if this player was able to play a tile
                ...
                if(game.state.tile_bank.length !== 0){
                    ...
                    newTile.predicted_type = window.sharedGameFunctions.predictTileType(game.state.board, game.state.chains, game.state.available_chains, newTile.x, newTile.y);
        ...
        shouldAutoPass: function(game){
            ...
            {
                if(!window.sharedGameFunctions.gameIsEndable(game)){
                    return true;
                }
            }
            return false;
            ...
            */
            ...
            switch(updateType){
                case 'playTile':
                    ...
                    if(['z', 'd'].includes(window.sharedGameFunctions.predictTileType(game.state.board, game.state.chains, game.state.available_chains, updateData.x, updateData.y))){
                            ...
                            if(this.shouldAutoPass(game)){
                                ...
                                game.state.player_states.forEach((player) => {
                                    player.tiles.forEach((tile) => {
                                        tile.predicted_type = window.sharedGameFunctions.predictTileType(game.state.board, game.state.chains, game.state.available_chains, tile.x, tile.y);
                                    });
                                });
                                this.endTurn(game);
                            }
